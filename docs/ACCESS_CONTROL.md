...
- IT Helpdesk: shortcut/create access for the SmartFlow `IT Helpdesk` workflow and IT dashboard
...
เมนูใน `resources/views/layouts/app.blade.php` และ backend checks ใน controllers ใช้ permission keys เดียวกัน เช่น ถ้าปิด `tickets.manage` ผู้ใช้จะไม่เห็นเมนูศูนย์ IT เดิม ส่วนงาน IT ใหม่ให้จัดการผ่าน `workflows.manage` เพราะ `/tickets` เป็น shortcut เข้า SmartFlow `IT Helpdesk` workflow แล้ว
